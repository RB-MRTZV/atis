...
This is an automated message from the RDS Scheduler.
        """
        
    def send_success_notification(self, action, summary, table_report=""):
        """Send a success notification.
        
        Args:
            action (str): Action performed (start/stop)
            summary (str): Summary of results
            table_report (str): Optional detailed table report
            
        Returns:
            bool: Success or failure
        """
        subject = f"RDS Scheduler {action.title()} - Success"
        message = self.format_email_report(summary, table_report)
        return self.send_notification(subject, message)
        
    def send_failure_notification(self, action, error_message):
        """Send a failure notification.
        
        Args:
            action (str): Action attempted (start/stop)
            error_message (str): Error details
            
        Returns:
            bool: Success or failure
        """
        subject = f"RDS Scheduler {action.title()} - Failed"
        message = f"""
RDS Scheduler Failure Report
