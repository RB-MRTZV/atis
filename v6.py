...
def analyze_resources():
    ...
    env_service_costs = defaultdict(lambda: defaultdict(float))
    env_service_counts = defaultdict(lambda: defaultdict(int))
    ...
    for resource in all_resources:
        ...
        env_service_counts[env][service] += 1
        ...
        env_service_costs[env][service] += instance_cost
    ...
    for env, services in env_instance_counts.items():
        ...
        # Add service breakdown for this environment
        for service, count in env_service_counts[env].items():
            service_cost = env_service_costs[env][service]
            
            env_data["service_breakdown"].append({
                "service": service,
                "count": count,
                "cost": round(service_cost, 2),
                "percentage": round((service_cost / env_costs[env] * 100) if env_costs[env] > 0 else 0, 2)
            })
        
        # Sort service breakdown by count
        env_data["service_breakdown"].sort(key=lambda x: x["count"], reverse=True)
    ...
    # Build flat table of instance counts (for JSON output and CSV)
    for env, services in env_instance_counts.items():
        for service, instance_types in services.items():
            for instance_type, count in instance_types.items():
                instance_cost_per_unit = cost_data.get(service, {}).get(instance_type, 100.00)
...
# Function to generate a CSV file with the same format as the console table
def generate_csv_table(results):
    # Create CSV file
    csv_filename = 'aws_instance_analysis.csv'
...
def main():
    ...
    # Print environment summary with service breakdown
    print("\n--- ENVIRONMENT SUMMARY ---")
    for env in results["environments"]:
        print(f"\n{env['name']} Environment:")
        print(f"  Total: {env['instance_count']} instances, ${env['current_monthly_cost']:.2f}/month, ${env['estimated_monthly_savings']:.2f} potential savings ({env['percentage_of_total_cost']:.2f}% of total cost)")
        
        # Print service breakdown
        print("  Service breakdown:")
        for service_data in env["service_breakdown"]:
            print(f"    - {service_data['count']} {service_data['service']} instances: ${service_data['cost']:.2f}/month")
        
        # Print instance type breakdown within each service
        print("  Instance type breakdown:")
        for service in env["services"]:
            for instance in service["instance_types"]:
                print(f"    - {service['name']} {instance['type']}: {instance['count']} instances, ${instance['current_monthly_cost']:.2f}/month")
    ...
    # Generate the main CSV table with the same format as the console output
    generate_csv_table(results)
    
    # Save results to JSON file
    json_output_file = 'aws_instance_analysis.json'
    with open(json_output_file, 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\nAnalysis complete. Results saved to {json_output_file}")
